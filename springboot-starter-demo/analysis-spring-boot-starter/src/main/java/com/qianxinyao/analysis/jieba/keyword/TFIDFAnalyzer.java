...
public class TFIDFAnalyzer {
	...
	private static final String BASE_DICTS_PATH = "/dicts";
...
//		System.out.println("加载自定义词库的信息，开始做自定义词库的分词------->>>>>>>>");
		Path path = Paths.get(new File(getClass().getClassLoader().getResource("dicts/iiasaas.dict").getPath()).getAbsolutePath());
		// 加载自定义的词典进词库
	...
	public List<Keyword> analyze(String content, int topN) {
		if (null != content && !"".equals(content.trim())) {
			content = content.trim().toLowerCase(Locale.getDefault());
		}
		...
		if (stopWordsSet == null) {
			...
			loadStopWords(stopWordsSet, this.getClass().getResourceAsStream(BASE_DICTS_PATH + "/stop_words.txt"));
		}
		if (idfMap == null) {
			idfMap = new HashMap<>();
			loadIDFMap(idfMap, this.getClass().getResourceAsStream(BASE_DICTS_PATH + "/idf_dict.txt"));
		}
...
//		String content = "312312312fasdfs我塑料管件来到北京清华大学北京中医医院小清新安培表1BOD1";
		String content = "北京清华大学1BOD1长春锅炉仪表";
//		String content = "重载111接插件HDC365";
		int topN = 20;
